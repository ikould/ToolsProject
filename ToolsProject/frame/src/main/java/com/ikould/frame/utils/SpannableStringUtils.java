    ...
    public static class Builder {
        ...
        private void setSpan() {
            ...
            if (imageIsBitmap || imageIsDrawable || imageIsUri || imageIsResourceId) {
                if (imageIsBitmap) {
                    mBuilder.setSpan(new ImageSpan(Utils.getContext(), bitmap, ImageSpan.ALIGN_BASELINE), start, end, flag);
                    bitmap = null;
                    imageIsBitmap = false;
                } else if (imageIsDrawable) {
                    mBuilder.setSpan(new ImageSpan(drawable, ImageSpan.ALIGN_BASELINE), start, end, flag);
                    drawable = null;
                    imageIsDrawable = false;
                } else if (imageIsUri) {
                    mBuilder.setSpan(new ImageSpan(Utils.getContext(), uri, ImageSpan.ALIGN_BASELINE), start, end, flag);
                    uri = null;
                    imageIsUri = false;
                } else {
                    mBuilder.setSpan(new ImageSpan(Utils.getContext(), resourceId, ImageSpan.ALIGN_BASELINE), start, end, flag);
                    resourceId = 0;
